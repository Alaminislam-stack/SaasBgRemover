...
const BuyCredit = () => {
  ...
  const handlePurchase = () => {
    toast.error("This feature is not available yet. Coming soon!");
  }
    ...
    <div className="h-screen lg:py-30 py-10 lg:px-[170px] px-10 ">
      ...
      <div className="flex flex-wrap justify-center items-center gap-5 mt-20">
        {plans.map((item, index) => (
          <div key={index} className=" w-[25%] bg-white px-10 py-10 rounded-lg border border-gray-300 drop-shadow-lg hover:scale-105 transition-all duration-500">
            ...
            <button onClick={() => handlePurchase()} className=" mt-2 w-full py-2 px-5 rounded-lg text-white bg-[#1f2937] cursor-pointer">Purchase</button>
          </div>
        ))}
      </div>
    </div>
